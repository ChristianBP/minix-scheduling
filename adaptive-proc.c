 ...
 */
  register struct proc *rp, *rp_walker;			/* process to run */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	rp_walker = rp;
	while( rp_walker != rdy_tail[q] ){
		if( rp->decaying_average_user_time > rp_walker->decaying_average_user_time ){
			rp = rp_walker;
		}
		rp_walker = rp_walker->p_nextready;
	}
